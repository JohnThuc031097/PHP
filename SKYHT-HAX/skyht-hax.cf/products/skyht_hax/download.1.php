...
<head>
	...
	<link
		...
		rel="stylesheet">
	<script async="" src="https://www.googletagmanager.com/gtag/js?id=UA-73797495-10"></script>
	<script>
		...
		<div class="container">
			...
			<div class="site-index">
				<div class="jumbotron">
					...
					<p itemprop="description">Tại đây bạn có thể tải về hack SkyHT-Hax và có thể sử dụng gần như ngay lập tức
					</p>
				</div>
				<div class="tab-nav">
					<ul>
						...
						<li role="presentation"><a href="download.php" class="tab-green-active tab-active">Tải về</a></li>
							...
							<div class="row">
								<div class="col-md-12">
									...
									<div class="row margin-md">
										<div class="col-md-5">
											<a target="_blank" href="https://mshare.io/file/fiSMscC" class="button-download button-download-green platform-win btn-block"
												role="button"><span><strong>SkyHT-Hax_FULL.zip</strong>
													<span>v1.0 | Link bản FULL</span>
											</a>
										</div>
										<div class="col-md-5">
											<a target="_blank" href="https://files.pw/nffxye5xs2bs" class="button-download button-download-blue platform-win btn-block"
												role="button"><span><strong>SkyHT-Hax_LITE.zip</strong>
													<span>v1.0 | Link bản LITE</span>
											</a>
										</div>
									</div>
									<h3><span class="icon icon-checkmark" aria-hidden="true"></span> Yêu cầu phần cứng</h3>
										<ul>
											<li>
												Chỉ hỗ trợ cho hệ điều hành Windows, bao gồm
												<ul>
													<li>Windows 10, 8.1, 8, 7, Vista, XP (32 / 64 bit)</li>
													<li>Windows Server 2012, Windows Server 2008, Windows Server 2003 (32 / 64 bit)</li>
												</ul>
											</li>
											<li>
												<strong style="color:red">Lưu ý</strong>
					 							<ul>
													<li>Nếu mở tool hack hiện thông báo "Server SkyHT-Hax bảo trì": Thử mở lại tool hack bằng quyền <span style="color:red">Run as administrator</span></li>
					 								<li>Nếu vào game không hiện menu hack. Vui lòng cài thêm 2 phần mềm này (<a href="" download="download/vcredist_x86.exe" role="button"><span style="color:royalblue">Phần mềm 1</span></a>, <a href="" download="download/dotNetFx45_Full_setup.exe" role="button"><span style="color:slateblue">Phần mềm 2</span></a>)</li>
												</ul>
											</li>
										</ul>
									<!-- Skills HACK 
									<div class="row">
										<div class="col-md-12">
											<div class="col-md-6">
												<h3><span class="icon icon-cancel" aria-hidden="true"></span> Các chức năng bản FREE </h3>
												<ul>
													<?php 
														$skills_new = isset($_SESSION['info_skills_hack'][0][0]) ? explode(';',$_SESSION['info_skills_hack'][0][0]) : null;
														$skills_hot = isset($_SESSION['info_skills_hack'][0][1]) ? explode(';',$_SESSION['info_skills_hack'][0][1]) : null;
														$skills = isset($_SESSION['info_skills_hack'][0][2]) ? explode(';',$_SESSION['info_skills_hack'][0][2]) : null;
					 									if(!is_null($skills_new)){
															for($i=0;$i<count($skills_new);$i++){
																echo '<li><code><span style="color:blue;">[New] '.$skills_new[$i].'</span></code></li>';
															}
														}
														if(!is_null($skills_hot)){
															for($i=0;$i<count($skills_hot);$i++){
																echo '<li><code><span style="color:red;">[Hot] '.$skills_hot[$i].'</span></code></li>';
															}
														}
														if(!is_null($skills)){
															for($i=0;$i<count($skills);$i++){
																echo '<li><code><span style="color:black;">'.$skills[$i].'</span></code></li>';
															}
														}
													?>
												</ul>
											...
											<div class="col-md-6">
											<h3><span class="icon icon-checkmark-circle" aria-hidden="true"></span> Các chức năng bản VIP </h3>
												<ul>
												<?php 
														$skills_new = isset($_SESSION['info_skills_hack'][1][0]) ? explode(';',$_SESSION['info_skills_hack'][1][0]) : null;
														$skills_hot = isset($_SESSION['info_skills_hack'][1][1]) ? explode(';',$_SESSION['info_skills_hack'][1][1]) : null;
														$skills = isset($_SESSION['info_skills_hack'][1][2]) ? explode(';',$_SESSION['info_skills_hack'][1][2]) : null;
					 									if(!is_null($skills_new)){
															for($i=0;$i<count($skills_new);$i++){
																echo '<li><code><span style="color:blue;">[New] '.$skills_new[$i].'</span></code></li>';
															}
