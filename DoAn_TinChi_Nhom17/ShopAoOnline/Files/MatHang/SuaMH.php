									...
									if($result){
										while($rows=mysql_fetch_row($result)){
											...
											echo 	'<td class="tbl-quantri-midle-right-tblChild-body-td-click">
														<a href="SuaMH-ChiTiet.php?mamh='.base64_encode($rows[0]).'&offset='.$offset.'&page='.$page.'">Chỉnh sửa</a>
													</td>';
											echo '</tr>';
										}
									}else{
                                ...
                                <tr align="right" bgcolor="#FFFFFF">
                                	<td colspan="3" class="c2-2">
                                    	<?php 
											...
											for($i=0; $i<$total/2; $i++){
												if(($i+1)==$page)
													echo '<a href="SuaMH.php?offset='.($i+$c).'&page='.($i+1).'"><blink style="color:red">'.($i+1).'</blink></a>&nbsp;';	
												else
													echo '<a href="SuaMH.php?offset='.($i+$c).'&page='.($i+1).'">'.($i+1).'</a>&nbsp;';
												$c++;
											}
										?>
                                    </td>
